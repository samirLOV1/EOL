...
```{r moving average & mann kendall EOL temp, echo=FALSE, warning=FALSE, message=FALSE}
...
ts_temp_eol <- ts(EOL_RAW$temperature, start = c(2018,1), end = c(2022,52), freq = 52)
...
plot_ts_temp_eol_less_season <- autoplot(ts_temp_eol_less_season) +
  ...
  ggtitle("Temperature time series at Pt B (without seasonality)") +
  geom_smooth(method=lm) +
  annotate(geom="text", x=2021, y=14.5, label="slope :  -0.10 ± 0.003 (significative, P <2e-16)", color="black")
...
```{r moving average & mann kendall EOL sal, echo=FALSE, warning=FALSE, message=FALSE}
...
#Transfome data into TS, frequency : 365.25/7 = 52.17857
ts_sal_eol <- ts(EOL_RAW$salinity, start = c(2018,1), end = c(2022,52), freq = 52)
...
```{r moving average & mann kendall EOL oxy, echo=FALSE, warning=FALSE, message=FALSE}
...
#Transfome data into TS, frequency : 365.25/7 = 52.17857
ts_oxy_eol <- ts(EOL_RAW$oxygen, start = c(2018,1), end = c(2022,52), freq = 52)
